...
export default function SentenceSetManager() {
  ...
  const handleEdit = (item: SentenceSet) => {
    // 打开对话框前重新加载目录,确保获取最新数据
    loadCatalogs()
    setEditingItem({
      id: item.id,
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('确定要删除此句子集吗?')) return

    try {
      const res = await fetch(`/api/admin/sentence-set?id=${id}`, { method: 'DELETE' })
      const data = await res.json()
      if (data.success) {
        toast.success('删除成功')
        loadSentenceSets()
  ...
  const handleSave = async () => {
    if (!editingItem || !editingItem.name || !editingItem.slug || !editingItem.catalogFirstId || !editingItem.ossDir) {
      toast.error('请填写必要信息')
      return
    }

    try {
      const res = await fetch('/api/admin/sentence-set', {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold">句子内容管理</h2>
                    ...
                    <Switch
                      ...
                      onCheckedChange={async (checked) => {
                        try {
                          ...
                          const data = await res.json()
                          if (data.success) {
                            setSentenceSets(prev => prev.map(s => s.id === item.id ? { ...s, isPro: checked } : s))
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingItem?.id ? '编辑' : '添加'}句子集</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="例如: sentences/daily"
              />
            </div>
            <div>
