...
export default function ShadowingSetManager() {
  ...
  const loadShadowingSets = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch(`/api/admin/shadowing-set?page=${page}&pageSize=${pageSize}`)
      const data = await res.json()
      if (data.success) {
  ...
  useEffect(() => {
    loadCatalogs()
    loadShadowingSets()
  }, [loadCatalogs, loadShadowingSets])
  ...
  const handleEdit = (item: ShadowingSet) => {
    loadCatalogs()
    setEditingItem({
      id: item.id,
      name: item.name,
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('确定要删除此跟读集吗?')) return

    try {
      const res = await fetch(`/api/admin/shadowing-set?id=${id}`, { method: 'DELETE' })
      const data = await res.json()
      if (data.success) {
        toast.success('删除成功')
        loadShadowingSets()
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch('/api/admin/shadowing-set', {
        ...
        body: JSON.stringify({
          ...editingItem,
          ossDir: editingItem.ossDir || `shadowings/${editingItem.slug}`
        })
      })
    ...
    <div className="p-6">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingItem?.id ? '编辑' : '添加'}跟读集</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="例如: shadowing-daily"
              />
            </div>
            <div>
            ...
            <div>
              ...
              <Input
                ...
                placeholder={`默认: shadowings/${editingItem?.slug || '...'}`}
              />
            </div>
            <div>
