        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground">
          {hasReading && (
            <span className="flex items-center gap-1">
              <span className="inline-block w-3 h-0.5 rounded bg-indigo-500" /> 阅读
            </span>
          )}
          {hasListening && (
            <span className="flex items-center gap-1">
...
function ModeResultCard({
  ...
  if (!record) {
    return (
      <div className="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 flex flex-col items-center gap-3">
        <div className={isReading ? 'p-2 rounded-full bg-indigo-100 dark:bg-indigo-900/30' : 'p-2 rounded-full bg-emerald-100 dark:bg-emerald-900/30'}>
          <Icon className={isReading ? 'w-4 h-4 text-indigo-600 dark:text-indigo-400' : 'w-5 h-5 text-emerald-600 dark:text-emerald-400'} />
        </div>
        ...
        <Button
          onClick={onStart}
          size="sm"
          className={`mt-1 cursor-pointer text-white ${isReading ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-emerald-600 hover:bg-emerald-700'}`}
        >
          开始测评
        </Button>
      </div>
  ...
  return (
    <div className=" bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700">
      <div className="flex items-center gap-2 mb-3">
        <Icon className={isReading ? 'w-4 h-4 text-indigo-600 dark:text-indigo-400' : 'w-4 h-4 text-emerald-600 dark:text-emerald-400'} />
        <span className="text-base font-medium text-muted-foreground">{label}</span>
        <span className='text-sm text-slate-400'>({dayjs(record.createdAt).format('YYYY-MM-DD HH:MM')})</span>
      </div>
      <div className="flex items-end gap-2 mb-1">
        <span className={`text-4xl font-bold ${isReading ? 'text-indigo-600 dark:text-indigo-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
      ...
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          variant="ghost"
          size="sm"
          className={`text-xs cursor-pointer ${isReading ? 'bg-indigo-500 hover:bg-indigo-700' : 'bg-emerald-500 hover:bg-emerald-700'} text-white hover:text-white`}
        >
          <RefreshCcw />
          重测
        </Button>
      </div>
      <div className={`text-sm text-slate-500 ${isReading ? 'bg-indigo-100' : 'bg-emerald-100'} rounded-lg px-3 py-3 mt-4`}>
        ...
        <div className="bg-gradient-to-br from-indigo-50 to-indigo-50 dark:from-indigo-950/20 dark:to-indigo-950/20 rounded-2xl p-2 md:p-6 border border-indigo-100 dark:border-indigo-900/30">
          <div className="flex flex-col items-center text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full mb-8">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-lg p-4 border border-slate-200 dark:border-slate-700">
                <div className="text-3xl font-bold text-indigo-600 dark:text-indigo-400 mb-1">3~6</div>
                <div className="text-sm text-muted-foreground">分钟完成</div>
              </div>
              <div className="bg-white dark:bg-slate-800 rounded-lg p-4 border border-slate-200 dark:border-slate-700">
                <div className="text-3xl font-bold text-indigo-600 dark:text-indigo-400 mb-1">A1-C2</div>
            ...
            <div className="flex gap-4">
              <Button
                ...
                size="lg"
                className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-4 cursor-pointer"
              >
